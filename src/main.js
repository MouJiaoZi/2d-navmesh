...
/** 点集 */
// 工具函数，生成八边形障碍
const generateObstacle = (x, y, radius) => new Array(8).fill(null).map((_, index) => {
...
const checkIsObstacle = (triangle, obstacles) => obstacles.some((obstacle) => triangle.every((p1) => obstacle.some((p2) => p1.x === p2.x && p1.y === p2.y)));
// 工具函数，计算三角网格的重心
...
const isSameEdge = (edge1, edge2) => {
  const [p1, p2] = edge1;
  const [q1, q2] = edge2;
  return (p1.x === q1.x && p1.y === q1.y && p2.x === q2.x && p2.y === q2.y) || (p1.x === q2.x && p1.y === q2.y && p2.x === q1.x && p2.y === q1.y);
};
// 工具函数，获取某个网格的三个邻居网格（通过判断是否跟某个三角形有公共边）
const getNeighborNodes = (nodes, curNode) => nodes.filter((otherNode) => {
...
const aStar = ({startNode, endNode, nodes, startPoint, endPoint}) => {
  ...
  while (openList.length) {
    ...
    if (currentNode === endNode) { // 到达终点，回溯路径
      const nodePath = [];
    ...
    for (let neighborNode of neighborNodes) {
      ...
      const preCommonEdge = currentNode.parent ? getCommonEdge(currentNode, currentNode.parent) : new Array(2).fill(startPoint);
...
const funnel = ({startPoint, endPoint, commonEdge, leftPoints, rightPoints,}) => {
  ...
  while (!isSamePoint(smoothedPath[smoothedPath.length - 1], endPoint)) {
    ...
    const addLeftIndex = () => {
      leftMoved = true;
      leftIndex = (() => {
        if (leftIndex >= leftTotal) return leftIndex;
        let tempIndex = leftIndex + 1;
        while (tempIndex < leftTotal && lefts[tempIndex].disable) tempIndex++;
        return tempIndex;
      })();
    };
    // 后退直到非disabled节点
    ...
    const addRightIndex = () => {
      leftMoved = false;
      rightIndex = (() => {
        if (rightIndex >= rightTotal) return rightIndex;
        let tempIndex = rightIndex + 1;
        while (tempIndex < rightTotal && rights[tempIndex].disable) tempIndex++;
        return tempIndex;
      })();
    };
    ...
    const addIndex = () => {
      const leftIndexAtCommonEdge = commonEdge.findLastIndex(edge => edge.some(p => isSamePoint(p, lefts[leftIndex].point)));
      const rightIndexAtCommonEdge = commonEdge.findLastIndex(edge => edge.some(p => isSamePoint(p, rights[rightIndex].point)));
...
const getRenderPosition = ({x, y}) => ({
  x: (x / 100) * (width - padding * 2) + padding, y: (y / 100) * (height - padding * 2) + padding,
});
// 工具函数，保留几位
...
const drawEdge = ({nodes}) => {
  for (const node of nodes) {
    ...
    ctx.stroke();
    // 索引文字
    ctx.font = "5px Helvetica, sans-serif";
    ctx.fillStyle = "#888";
...
const main = ({startPoint, endPoint, canvas}) => {
  // 地图边界
  const map = [{x: 0, y: 0}, {x: 0, y: 100}, {x: 100, y: 0}, {x: 100, y: 100}];
  // 障碍
  const obstacles = (() => {
    const obstacles = [// generateObstacle(30, 46, 10),
      // generateObstacle(55, 40, 10),
      generateObstacle(40, 20, 10), generateObstacle(80, 21, 15), generateObstacle(15, 14, 10), generateObstacle(25, 70, 10), generateObstacle(60, 60, 16), generateObstacle(85, 80, 10), generateObstacle(20, 45, 6),];
    return obstacles.map(ob => ob.map((p) => {
  ...
  const smoothedPath = funnel({
    startPoint, endPoint, commonEdge, leftPoints, rightPoints,
  });
...
let startPoint = {x: 35, y: 48};
let endPoint = {x: 70, y: 79};
